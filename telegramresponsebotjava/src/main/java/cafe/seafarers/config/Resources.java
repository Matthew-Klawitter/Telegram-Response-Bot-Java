...
public class Resources {
    ...
    /**
     * Loads a File contained within the Resources folder
     * Resource must be located at pluginName/filename
    ...
    /**
     * Streams a file contained within the Resources folder
     * Resource must be located at pluginName/Filename
     * @param plugin The plugin attempting to load a resource
     * @param fileName The filename of the resource to be loaded
     * @return BufferedReader containing the resource stream, null if it doesn't exist
     */
    public static BufferedReader LoadFileResourceReader(BotPlugin plugin, String fileName){
        try {
            ClassLoader cl = plugin.getClass().getClassLoader();
            InputStreamReader isr = new InputStreamReader(Objects.requireNonNull(cl.getResourceAsStream(plugin.getName() + "/" + fileName)));
